...
def _render_turnover_tab(SELLER_DIR, data_dir, selected_seller):
    ...
    if df_risk is not None and df_all is not None:
        ...
        if 'moving_avg_30d' in chart_data.columns:
            chart_data['moving_avg_30d'] = chart_data['moving_avg_30d'].fillna(0)
        ...
        if df_market is not None:
            ...
            if not market_data_temp.empty:
                max_mkt = market_data_temp['market_avg_sales'].max()
                if pd.notna(max_mkt):
                    max_y = max(max_y, max_mkt)

        if pd.isna(max_y) or max_y == 0: max_y = 10 
...
def _render_risk_tab(SELLER_DIR, data_dir, selected_seller, df_agg, df_tier):
    ...
    if df_sku is not None:
        ...
        # Display Metrics
        # Display Metrics (Custom HTML for Equal Height)
        st.markdown("""
        <style>
            .metric-grid {
                display: grid;
            ...
            .delta-neg { background-color: #dcfce7; color: #166534; } /* Good (Inverse) */
            .delta-pos { background-color: #fee2e2; color: #b91c1c; } /* Bad (Inverse) */
        ...
        if delta_val:
            try:
                # delta_val format: "-2.5일 (vs Tier 1)" or "+1.2일..."
                val_num = float(delta_val.split('일')[0])
                # inverse: negative is good (green), positive is bad (red)
                delta_class = "delta-neg" if val_num < 0 else "delta-pos"
                delta_arrow = "↓" if val_num < 0 else "↑"
                delta_html = f'<div class="metric-delta {delta_class}">{delta_arrow} {delta_val}</div>'
            except:
                delta_html = f'<div class="metric-delta">{delta_val}</div>'

        st.markdown(f"""
        <div class="metric-grid">
            <div class="metric-card">
                <div class="metric-label">판매자 등급</div>
                <div class="metric-value">{tier_val}</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">총 주문 건수</div>
                <div class="metric-value">{int(total_orders):,}건</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">월 평균 주문</div>
                <div class="metric-value">{int(monthly_avg):,}건</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">고객 만족도</div>
                <div class="metric-value">{avg_review:.1f}</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">평균 출고 시간</div>
                <div class="metric-value">{avg_handling:.1f}일</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">다빈도 배송지</div>
                <div class="metric-value">{top_state}</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">평균 배송 시간</div>
                <div class="metric-value">{avg_delivery:.1f}일</div>
                {delta_html}
            </div>
            <div class="metric-card">
                <div class="metric-label">주력 카테고리</div>
                <div class="metric-value" style="font-size: 14px; word-break: break-all;">{top_cat}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
